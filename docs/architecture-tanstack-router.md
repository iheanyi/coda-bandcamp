# TanStack Router architecture for Coda

Status: implemented on 2026-08-11. This decision record applies TanStack Router
v1's React architecture to Coda's Tauri desktop renderer and documents the
resulting route, data, preload, focus, scroll, and code-splitting contracts.

## Accepted decision

Coda uses TanStack Router's recommended **file-based routing** with its Vite
plugin, automatic code splitting, and `createHashHistory` for the main Tauri
window. File routes and the generated tree are the sole destination model.
...
The router plugin precedes React without changing Tailwind or React behavior:
...
The destination hierarchy lives under `src/routes/`. A directory route's
...
Coda enables `defaultPreload: "intent"`. Hover or touch intent on a `<Link>`
...
Coda keeps `transitionCodaView` as the single transition owner and passes typed,
Promise-returning router commits into it; Router does not start a nested View
Transition. Unique shared-element names, directional back/forward types, and
reduced-motion bypasses remain part of that boundary. [View-transition navigation](https://tanstack.com/router/latest/docs/api/router/NavigateOptionsType#viewTransition)
...
The Vite plugin uses `autoCodeSplitting: true` and splits each
...
Production verification checks emitted route chunks and intent-preload reuse.
Automatic splitting is a build transformation, so component tests alone do not
prove that packaged chunks are emitted or requested as expected.

## Implemented migration sequence

1. Added the Vite plugin, generated route tree, root route, router factory, and
   memory-history test harness.
2. Extracted the persistent `AppShell` and focused feature controllers while
   keeping player, queue, audio, bridges, dialogs, and toaster above the outlet.
3. Made the hash route the sole source of destination truth and removed the
   mirrored `view` state machine.
4. Replaced deterministic sidebar and in-content destinations with typed
   semantic `<Link>` elements using intent preloading.
5. Moved album, artist, Discover, Radio, playlist, and Now Playing details to
   validated path params while preserving transition and focus provenance.
6. Moved shareable collection controls to validated search params with history
   replacement for live query typing.
7. Added Query-backed route loaders and nearest pending, error, and not-found
   boundaries without copying remote payloads into Router cache.
8. Removed redundant destination-level lazy routing, verified automatic route
   chunks, and completed the native macOS transition smoke suite.
