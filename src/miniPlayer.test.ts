...
describe("mini player event contract", () => {
  ...
  it("validates unknown snapshots before the compact window renders them", () => {
    ...
    expect(parseMiniPlayerSnapshot({ ...valid, volume: Number.NaN })).toBeUndefined();
    expect(parseMiniPlayerSnapshot({ ...valid, volume: 2 })).toBeUndefined();
    expect(parseMiniPlayerSnapshot({
      ...valid,
  ...
  it("accepts only bounded compact-player commands", () => {
    expect(parseMiniPlayerCommand({ type: "play-pause" }))
      .toEqual({ type: "play-pause" });
    expect(parseMiniPlayerCommand({ type: "previous" }))
      .toEqual({ type: "previous" });
    expect(parseMiniPlayerCommand({ type: "next" }))
      .toEqual({ type: "next" });
    expect(parseMiniPlayerCommand({ type: "show-main" }))
      .toEqual({ type: "show-main" });
    expect(parseMiniPlayerCommand({ type: "seek", positionSeconds: 73.5 }))
      .toEqual({ type: "seek", positionSeconds: 73.5 });
    expect(parseMiniPlayerCommand({ type: "volume", volume: 0.35 }))
      .toEqual({ type: "volume", volume: 0.35 });

    expect(parseMiniPlayerCommand({ type: "seek", positionSeconds: -1 }))
      .toBeUndefined();
    expect(parseMiniPlayerCommand({ type: "seek", positionSeconds: Number.NaN }))
      .toBeUndefined();
    expect(parseMiniPlayerCommand({ type: "volume", volume: 1.01 }))
      .toBeUndefined();
    expect(parseMiniPlayerCommand({ type: "queue-everything" }))
      .toBeUndefined();
  });
});
