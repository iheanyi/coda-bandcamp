...
function NowPlayingViewComponent({
  ...
  const repeatLabel =
    repeat === "off" ? "Repeat off" : repeat === "all" ? "Repeat queue" : "Repeat current track";
  const radioTimeline = useMemo(
    () => boundRadioChapters(track.radioChapters ?? []),
    [track.radioChapters],
  );
  const radioAiring = useMemo(
        ...
        >
          ...
          <ol className="now-playing__radio-chapters">
            {radioTimeline.map((chapter, index) => {
              const isCurrent = chapter === radioAiring.current;
              const isNext = chapter === radioAiring.next;
              return (
                <li
                  className={[
