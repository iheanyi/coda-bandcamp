    ...
    >
      <SliderPrimitive.Control className="relative flex w-full touch-none items-center select-none data-disabled:opacity-50 data-vertical:h-full data-vertical:min-h-40 data-vertical:w-auto data-vertical:flex-col">
        <SliderPrimitive.Track
          ...
          className="relative grow overflow-hidden rounded-full bg-[#3a3d3f] select-none data-horizontal:h-[3px] data-horizontal:w-full data-vertical:h-full data-vertical:w-[3px]"
        >
        ...
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb
            ...
            className="relative block size-[10px] shrink-0 rounded-full border-0 bg-[#e9e7e1] opacity-0 outline-none transition-opacity duration-(--duration-coda-fast) select-none after:absolute after:-inset-2 group-hover/slider:opacity-100 has-[input:focus-visible]:opacity-100 has-[input:focus-visible]:outline-2 has-[input:focus-visible]:outline-ring has-[input:focus-visible]:outline-offset-2 data-disabled:pointer-events-none data-disabled:opacity-0 motion-reduce:transition-none"
          />
        ))}
