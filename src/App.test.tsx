...
describe("Coda application flows", { timeout: 10_000 }, () => {
  ...
  it("batches a cold single-track shuffle tail before persisting structural growth", async () => {
    const shuffleAlbums: Album[] = Array.from({ length: 8 }, (_, index) => ({
      ...album,
      id: `cold-batch-album-${index}`,
      title: `Cold Batch Album ${index}`,
      songCount: 1,
      tracks: undefined,
    }));
    const requests = new Map(shuffleAlbums.map((release) => [
    ...
    const resolveCall = (index: number) => {
    ...
    await screen.findByText("Cold Batch Album 0");
    await startArtistShuffle();
    await waitFor(() => expect(mocks.fetchAlbum).toHaveBeenCalledTimes(4));
    await act(async () => resolveCall(0));
    expect(await screen.findByRole("button", { name: "Pause" }))
      .toBeInTheDocument();
    await waitFor(() => expect(mocks.savePlayerState).toHaveBeenCalledOnce(), {
      timeout: 1_500,
    });
    ...
    expect(await screen.findByText("3 tracks next")).toBeInTheDocument();
    await waitFor(() => expect(mocks.savePlayerState).toHaveBeenCalledOnce(), {
      timeout: 1_500,
    });

    await waitFor(() => expect(mocks.fetchAlbum).toHaveBeenCalledTimes(8));
    await act(async () => {
      for (let index = 4; index < 8; index += 1) resolveCall(index);
    });
    expect(await screen.findByText("7 tracks next")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Pause" })).toBeInTheDocument();
  });
